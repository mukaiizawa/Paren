...
上記のプログラムを評価すると二行出力される。一行目がwriteがコンソールへ印字した文字列であり、二行がwriteの評価結果をreplが印字したものである。
...
関数の本体では、read-byteで読み込んだバイトでシンボルchを束縛している。その後、chを参照してwrite-byteで読み込んだバイトを出力している。
...
## 結合
文字列の結合はstringを使用する。stringは可変長の引数の印字表現を返す。
...
同名のシンボルはシステム上でただ一つしか存在しないため同一性の検証はアドレス比較で高速に行える。
...
# キーワード
キーワードは束縛できない点を除いてシンボルと同じでデータ型である。
...
これらの関数(cxx...xr y)は(cxr (cxr ... (cxr y)))と等価である。
...
### シンボル
先頭要素がシンボルの場合、期待される値は以下のいずれかである。それぞれの規則にしたがってリストが評価される。
