...
- unless
## if
ifはParenで最も基本となる条件分岐である。
最初の引数が真の場合にのみ第二引数が評価され、
そうでなければ:nilが返る。
    (if 3 :true)
    => :true
    (if :nil :true)
    ...
    (if (list) :true)
    => :nil
先に述べたように、Parenでは:nil以外の値は真と見做される。
次に示す通り、偽の場合は第二引数は評価されない。
    (/ 1 0)
    => Error
    (if :nil (/ 1 0))
    => :nil
この動作は他のどの条件分岐でも同じである。
ifには三つまで引数が渡せてる。
その場合、条件式が偽の場合に最後の引数が評価された値が返る。
    (if 3 :true :false)
    => :false
    (if (cdr (cons 1 :nil)) :true :false)
    => :false
条件分岐した際に複数の式を評価したいかもしれない。
その場合は後述するスペシャルフォームbeginを使用する。
    (if :t
      (begin (put "t")
             (put "r")
             (put "u")
    ...
    => true
## cond
condは複数の分岐を行う場合にしばしば用いられる。
次のような場合、ifを使用すると可読性が悪い。
    (if (= x 1)
      :one
      (if (= x 2)
        :two
        (if (= x 3)
          :three
          :else)))
これは、condを使用すると次のように書ける。
    (cond ((= x 1) :one)
          ((= x 2) :tow)
          ((= x 3) :three)
          (:t :else))
condの引数はすべてリストで、最初の引数から順に次の処理を行う。
1. 引数のリストの第一要素を評価する。
2. 引数の第一要素が偽の場合は次の引数に対して1から処理を行う。
   ただし、この引数が最後の引数の場合はcondは:nilを返す。
3. 引数の第一要素が真の場合は、そのリストの残りの要素を逐次評価し、
   condは最後に評価した値を返す。
## whenとunless
whenは条件式を返した場合に複数の式を評価する場合に用いる。
    (when (= x 1)
      (put "t")
      (put "r")
      (put "u")
...
意味論的に次の式は等価である。
    (if (= x 1)
      (begin (put "t")
             ...
             (put "u")
             (put "e")))
unlessは条件式が偽を返した場合に複数の式を評価する。
これは、後述するnotを使用して次の式と意味上は等価である。
    (unless 条件式
      式)
    <=>
    (when (not 条件式)
