...
ParenはS式によって記述されるプログラミング言語Lispの方言である。オブジェクトシステムなど着想の多くはCommon　Lispから受けている。Common Lispと最も異なる特徴を一つ上げるなら、Parenは手続き型言語としての側面が強いことである。一般に、代入が少ないプログラムはそうでない場合に比べ堅牢である一方で冗長になりがちである。Parenは代入のリスクを自己管理でき、必要に応じて使用する玄人のための言語である。勿論、Lispユーザを魅了してやまない玄人のための機能、マクロも使える。
...
という性質を用いてより簡潔に書ける。
代入オペレータ―は代入した値を返す。その性質を利用するとより簡潔に記述することができる。
    ; copy 第二版
    (function copy ()
...
# 数値
Parenでは、すべての実数は数値型となる。内部的には、すべての数値は浮動小数点数で表される。
## 数値リテラル
数値リテラルの書式を示す。
    [base x] number
    base -- 基数
    number -- 実数
基数を指定しない場合は10進数と見做される。また、基数に0を指定した場合は16進数と見做される。
## 評価
数値は評価されると自身を返す。
    ) 10
    10
...
# キーワード
キーワードは同名であるならばプログラムでただ一つだけ存在するという性質を持つ。キーワードを作成するには次のいずれかの方法を用いる。
- キーワードリテラルを使用する
- to_key関数を使用する
...
TODO:
リストの説明をするにはどうやってもnilが必要なため、変数の章をここにもってくるか。
...
空を表す擬似変数nilに対してlist?がtrueを返すように実装されている。
