...
# 変数
Parenには値を設定、参照する仕組みがある。
その仕組みは変数という機能により実現される。
## 変数名
...
## 変数の束縛
変数に値を代入するには代入オペレーター`<-`を使用する。
    (<- a 3)
    => 3
    (* a 4)
    => 12
例のように、代入オペレーターは最後の変数に代入した値を返す。
...
quoteは引数を評価せずに、そのまま返す。
    ...
    (quote a)
    => a
quoteはParenで頻繁に使用するため構文糖が用意されてある。
構文糖を使用するには、評価したくない式にバッククォートを前置する。
    a
    => 3
    `a ; <=> (quote a)
    => a
