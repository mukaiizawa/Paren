...
クラスは名前、スーパークラス、インスタンス変数を指定して作成する。スーパークラスについては後述する。

例として二次元ベクトル空間の元を表すクラスPointを示す。クラス名は慣習としてパスカルケースで命名する。
    (class Point () x y)
...
自動生成されるアクセサはインスタンス変数名の前に`.`を付けたものとなる。そのため、メソッド以外の識別子を`.`から始めることは推奨しない。
...
例のように、インスタンス変数の更新式もインスタンス変数の値が返される。
...
methodマクロは仮引数リストの必須パラメーターに少なくとも一つクラスとシンボルの対を渡す点を除いて、スペシャルフォームfnと同じである。

methodマクロはグローバルシンボル$methodにメソッド登録するS式に展開される。

POSでは慣習として初期化メソッドを.initという名前で作成し、オブジェクトの生成時に呼ぶ。

Pointクラスの意味のある初期化処理は二つ考えられる。即ち、任意の元と零元である。これらは次のように書ける。
...
二次元ベクトル空間上の元の和を返すメソッドを例として示す。これは、初期化メソッドを用いて次のように書ける。
...
POSのメソッドは、名前及び、順序を加味した必須パラメーターの型の組で一意でなければならない。例えば、先の.addに加えて次のメソッドは定義することはできない。
...
また、型判定マクロtype?はスーパークラスに対しても真を返す。
    ) (type? (new B) :A)
    true
    ) (type? (new C) :D)
    false
...
## メソッドのディスパッチ
メソッド呼び出しは、仮引数の必須パラメーター部と実引数がすべてtype?マクロで真を返さなければならない。
