...
2. 作成した文字列を印字する。
文字列を作成する一番簡単な方法は文字列リテラルを使用することである。
文字列リテラルはダブルクォーテーションで括られた文字の列である。
    ) "hello world"
    "hello world"
例のように、文字列は評価されると自身を返す。
replが`hello world`を印字したので課題は終了である。
自力で標準出力を印字するにはprintln関数が使える。
    ) (println "hello world")
    hello world
    "hello world"
...
関数の本体では、chという変数にread_charで標準入力から取得した文字を代入している。
そのあとに、chに代入された文字をprintで標準出力に書き出している。
このプログラムは、
