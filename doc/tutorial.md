...
# 概要
このチュートリアルはParenの基本文法を早巡りすることを目的とする。いくつかの専門用語は定義することなしに使用しているため、多少の計算幾何学の知識を前提としている。

ParenはS式によって記述されるプログラミング言語Lispの方言である。ほかのLispの方言と比べると、より手続き型の言語となっている。
...
# 序章
この章ではいくつかの簡単なプログラムを示し、Parenの雰囲気を知ってもらうことを目的とする。

## Parenの実行
引数を与えることなしにParenを実行すると、repl(read-eval-print-loop)が起動する。
    $ paren
    ) 
...
例として簡単な四則演算を評価する。
    ...
    36
式が入力されると、式を読み込み(read)、評価(eval)して、その結果を印字(print)する。これが繰り返される(loop)ためreplという。
...
1. 文字列'hello world'を作成する。
2. 作成した文字列を印字する。
文字列を作成する一番簡単な方法は文字列リテラルを使用することである。文字列リテラルはダブルクォーテーションで括られた文字の列である。
    ) "hello world"
    "hello world"
例のように、文字列は評価されると自身を返す。replが`hello world`を印字したので課題は終了である。明示的に印字するにはwrite_line関数が使える。
...
関数の本体では、chという変数にread_charで標準入力から取得した文字を代入している。そのあとに、chに代入された文字をwrite_charで標準出力に書き出している。
このプログラムは、
