...
`=`は二つ以上のオブジェクトがすべて同等であれば真を返す。
    ...
    (= a b)
    => :t
ただし、比較するオブジェクトの型が異なる場合は必ず偽を返す。
    (= "3" 3)
    => :nil
`eq?`は、比較する二つ以上のオブジェクトがすべて同一である場合に真を返す。
そのため、`eq?`で真を返すならば`=`は必ず真を返すが逆はそうとは限らない。
    (eq? 3 3)
    => :nil
    (<- a 3 b 3)
    => 3
    ...
    (eq? a b)
    => :t
`eq?`は番地の比較をするだけなので高速である。
また、Parenでは同一変数はすべて同じオブジェクトであることが保証されている。
    (eq? :a :a)
    => :t
    (<- a 3)
    (eq? a a)
...
2. formを逐次実行し、1へ戻る
while全体として、result-formが指定されている場合は
## each
...
## TODO
反復の終了条件を指定しない場合のフォーム全体の返り値について仕様を決める。
...
ここでは、まだ説明していないスペシャルフォームについて述べる。
## let(変数束縛)
    let = '(let' '(' let_form+ ')'
              s_expr* ')'
