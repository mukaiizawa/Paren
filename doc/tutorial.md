...
# コメント
コメントはParenに無視されるため、プログラムの有用な補足事項等を自由に記述することができる。
## 一行コメント
...
キーワードへの参照を取得するためには次のいずれかの方法を用いる。
- キーワードリテラルを使用する
- to_key関数を使用する
## キーワードリテラル
...
環境とは、シンボルテーブルと零又は一つの環境への参照の対のことをいう。ここで、シンボルテーブルとはシンボルとオブジェクトへの参照の対の集合のことをいう。ただし、ある環境のシンボルテーブルに存在する同一シンボルは高々一つである。

環境の持つ環境への参照の実体を親の環境という。これに対して、ある親の環境の参照元の環境を子の環境という。

Parenには親の環境が存在しないような環境は高々一つしか存在しない。この環境のことを大域環境乃至、トップレベルという。

ある環境EのシンボルテーブルTにシンボルSとオブジェクトへの参照Pの対(S, P)が保持されているとき、EにSがPで束縛されているという。また、Tに対(S, P)を追加することを、EにSをPで束縛するという。これは、単にSをPで束縛する、乃至、Sを束縛するともいう。
...
シンボルの評価は次の手順で行われる。
    ...
    親の環境を辿っていき、大域環境のシンボルテーブルにもSが存在しない場合はエラーとする。

このように、あるシンボルが保持するオブジェクトへの参照は、どの環境下でシンボルが評価されたのか考えなければ意味がない。

例えば、前述した`+`は大域環境に束縛されているシンボルのことであり、評価されることによって引数の和を計算する関数が返っていたにすぎない。

## シンボルの束縛
シンボルを束縛するには束縛オペレーターを使用する。束縛オペレーターにはその用途から三種類用意されている。
    ...
    value -- シンボルに束縛する値
    result -- 最後に束縛した値
`<-`はシンボルと束縛する値の組を現在の環境に束縛する。既に現在の環境にシンボルが存在している場合は束縛されている値を更新する。

`<--`は現在の環境にシンボルが束縛されているか確認し、束縛されている場合はその値を更新する。束縛されていない場合は親の環境に対して再帰的に探索を続ける。大域環境にも束縛されていない場合は大域環境に束縛する。

`<---`は常に大域環境にシンボルを束縛する。既に束縛されている場合はその値を更新する。

`<-`と`<--`の束縛オペレーターの違いは今は不明瞭かもしれないが、次の簡単な方針に従って使っていれば問題ない。
`<-`は常に現在の環境にのみ干渉するという特徴から、局所的なシンボルを束縛するために使用する。`<--`は閉包を作成するときに使用する。

## シンボルの再束縛
子の環境で親の環境に束縛されているシンボルを束縛する行為をシンボルの再束縛、乃至、シャドーイングという。子の環境から親のシンボルが束縛されている値を参照できなくなることからシンボルを隠すともいう。

シンボルの再束縛は有用な用途があるためしばしば用いられるが、濫用しすぎるとプログラムの可読性が落ちるため注意がひつようである。　
## 環境の作成
Parenは関数オブジェクトが環境を持つ。つまり、新しく環境を作るということは関数オブジェクトを作成することに他ならない。このことについては関数の章で後述する。

## グローバルシンボル
大域環境に束縛されたシンボルのことをグローバルシンボルという。グローバルシンボルは再束縛されていないすべての環境から参照できる。
...
## 特殊なシンボル
システムであらかじめ組み込まれた参照しかできない特殊なシンボルが存在する。
...
## 定数
Parenでは言語機能として定数は存在しない。そのため、定数として扱うシンボルは、慣習として他のシンボルと見分けがつくように大文字のシンボル名を使用する。
    ) (<--- PI 3.141592)
    3.141592
