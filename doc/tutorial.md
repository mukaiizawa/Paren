...
# Hello Paren
Parenで標準出力に出力するには組み込みのput関数を使用する。
...
2. put関数の評価結果
を意味する。
このように、Parenは関数型言語なのですべての式は値を持つ。
Parenの世界へようこそ。
...
変数のスコープやエクステントについては、
後述する環境モデルなしに説明することは困難である。
そのため、この章では変数の簡単な使用例や、変数の種類について簡単に述べる。
## 変数名
...
次のいずれもParenの変数名として有効である。
    var
    x1
他言語では使えなさそうな記号も変数名に使用できることが多い。
このことは、Parenの構文が簡単であることの裏付けになっている。
次にあげる変数名は(推奨されるような名称かはさておき)すべて有効である。
変数名称で使用できる文字の厳密な定義はParen言語仕様書の字句解析の章を参考のこと。
    !$*+-0-9abc?
    m<__.__>m
Parenでは複数の単語で構成されている変数名は、
二つ目以降の単語の先頭文字を大文字にする。
    stringWriter
    arrayList
このような命名方法をキャメルケースという。
ただし、複数の語の頭文字をで構成されている単語を使用する場合は、
最初の文字のケースに合わせる。
    HTTPRequestHandrer
    readXML
...
このスペシャルオペレーターを以後代入オペレータ―という。
    (<- a 3)
    => 3
例のように、代入オペレーターは代入した値を返す。
この性質を利用すると、
標準入力から標準出力へ複写するプログラムは次のように書ける。
    (while (!= (<- c (getChar)) :EOF)
        (putChar c)
代入オペレーターは一度に複数の変数に値を代入できる。
その場合は、左から順に上記の処理が行われる。
     (<- x 3 y (* x 2))
     => 6
     (* x y)
     => 18
複数の変数に値を代入した場合の代入オペレータ―の返り値は、
最後に代入した値となるが可読性の問題からその値を使用することはお勧めしない。
## 参照
変数の値を参照するには、変数を評価すればよい。
    (<- a 3 b 4)
    => 4
    a
    => 3
...
Parenを使いこなすうえで、環境モデルを理解することは必須といえる。
## 環境モデル
環境とは、変数とその値の一対一の対応の集合である。
また、ある一つの環境を除き、
すべての環境にはその環境の上位の環境が必ず一つだけ存在する。
その、ある一つの特別な環境のことをトップレベルという。
Parenを起動した直後の環境はトップレベルである。
## 変数の束縛
ある環境の変数とそれに対応付けられた値との集合に、
新しく要素(変数k、値vの組)を追加する行為を、
その環境にkをvで束縛するという。
環境を新しく作成しない限り、すべての変数はトップレベルに束縛される。
## 代入と束縛
前述した代入を、環境モデルを考慮して次のように再定義する。
変数kに値vを代入するとは現在の環境に変数kを値vで束縛することである。
この定義により今までの代入の例は、
すべてトップレベルに変数を束縛してきたことにほかならない。
## 環境の作成
現在の環境の下に新しく環境を作るにはスペシャルオペレーターletを使用する。
まず最初にその使用例を示す。
...
次にそれらの積を評価し、結果が0なのが確認できる。
    ...
    (* a b c)
    => 0
次にletでトップレベルの下に新しく環境を構築し、
その環境でa、b、cをそれぞれ1、2、3で束縛している。
その新しい環境内でそれらの積を評価すると、
結果がトップレベルでの評価と異なっていることが確認できる。
    ...
    (* a b c)
    => 0
また、最後にletの外で再度積を評価すると結果が0になることが分かる。
このことから、新しい環境内での変数の束縛は、
外の環境に影響を及ぼさないことが分かる。
ここでletの書式を示す。
    (let (let_arg*)
        form*)
    let_arg := variable | (variable initial-value)
第一引数は作成する環境に束縛する変数または変数と初期値のリストのリストである。
第二引数以降は作成した環境内で評価するS式となる。
let全体は、先の例のように最後に評価されたS式の値を返す。
## 変数の評価
letの実行例からもわかるように、
ある変数kが束縛されている値は環境に依存するため、
環境のことを考えずに変数kが束縛されている値を考えることに意味はない。
ここで、現在の環境に変数が束縛されていない場合について考える。
トップレベルで変数が束縛されていない場合にエラーとなることはすでに述べた。
    a
    => Error
次に、トップレベル以外の環境でその環境に変数が束縛されていない例を示す。
トップレベル同様に、その場合はエラーとなる。
    (let ()
      a)
    => Error
ただし、その環境の上位の環境にその変数が束縛されている場合はその値が返される。
    (<- a 1)
    => 1
    (let ()
      a)
    => 1
次のように、上位の複数の環境に変数が束縛されている場合は、
...
## 変数束縛時の評価
letで新しく環境を作成する場合に、
依存関係のある変数を束縛する場合はどのような処理になるか知っておく必要がある。
次のような場合、let変数bはこの環境のaを参照して初期化処理を行う。
    (let ((a 0))
      (let ((a 1)
            (b a))
        b))
    => 1
また、変数束縛のリストは左から順に評価されることが保証されている。
この動作は伝統的なLisp系言語の`let*`と同じである。

# 数値
いままでさりげなく算術演算を使用してきたが、
この章でそれらについて詳しく述べる。
## 評価
数値は評価されると、自身を返す。
    3
