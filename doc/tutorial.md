...
例として簡単な四則演算を評価する。
    ...
    36
式を入力すると読み込み(read)、評価(eval)して、印字(print)する。
これが繰り返される(loop)ためreplという。
...
明示的に印字するにはputln関数が使える。
    ) (putln "hello world")
    hello world
    "hello world"
putln呼び出しによる二つの行はそれぞれ
1. putlnが標準出力へ出力した結果
2. replが印字した式の評価結果
を意味する。
putln関数は引数を標準出力に印字すると同時に、評価結果として引数を返す。
