...
そしてそれは、大概次のプログラムを作成することから始まる。
次の文字列を印字せよ。
    hello world
Parenを起動するにはシェルの中でparenを実行すればよい。
その場合repl(read-eval-print-loop)と呼ばれる対話モードで起動される。
    $paren
    ) 
`)`がParenのプロンプトである。
標準出力に文字列を出力するにはprint関数を使用する。
関数を実行するには、実行する関数名とその引数を`()`で括る。
    ) (print "hello world")
    hello world
    "hello world"
Parenの構文はこれだけである。
ただし、後述するリードマクロにより構文を自由に拡張できる。
print呼び出しによる二つの行はそれぞれ
1. printが標準出力へ出力した結果
2. replが印字した式の評価結果
を意味する。
関数の引数が関数呼び出しの場合、
replを利用するとParenを電卓として扱うこともできる。
    ) (+ 3 4)
    7
関数は実行される前に、その引数すべてが評価される。
その評価結果が関数の引数として呼び出される。
    ) (* (+ 3 4) 2)
    14
...
初期化されていない変数の値は:nilとなる。
    (<- q (new Point :x 3 :y 4))
## インスタンスの型
インスタンスの型を調べるにはtypeマクロを使用する。
    (type class)
    class -- クラス名
クラスの型はクラス定義の際に、クラス名に対応する恒等変数が自動的に設定される。
    (type p)
    => :Point
