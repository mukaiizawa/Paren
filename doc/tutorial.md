...
Parenにはそのための関数`same?`が用意してある。

    ) (same? 'a 'a)
    true

    ) (same? 'a (string->symbol "a"))
    true
...
## キーワードの比較
シンボル同様、アドレスの比較で済むため`same?`を使用する。

    ) (same? :abc (string->keyword "abc"))
    true
...
キーワードパラメーターには初期値を与えることができ、呼び出し時にキーワードパラメーターが省略された場合は初期値が使用される。キーワードパラメーターが初期値なしで定義された場合は初期値にnilを指定したものと見做される。
    ...
    ) (function k1-k2-k3 (:key (k1 1) (k2 2) k3)
        (list k1 k2 k3))
    k1-k2-k3
    ) (k1-k2-k3)
    (1 2 nil)
...
## return(大域脱出)
returnは現在のlambdaコンテキストから大域脱出するためのスペシャルオペレーターである。

    ) ((lambda () 1 (return 2) 3))
    2

マクロにより、暗黙のlambdaで包まれている場合、想定した結果にならないことがあるため注意が必要である。
...
マクロはスペシャルオペレーターmacroを使って定義する。
    ...
    (macro name params body_form ...) => result
    params ::= ( { params | param } ... )
    param ::= sym ...  [:opt sym-val ...] [:rest sym] [:key sym-val ...]
    sym-val ::= { sym | (sym val) }
    name -- 定義するマクロ名
    sym -- シンボル
    val -- 値
...
メソッドの定義はmethodマクロを使用する。
    ...
    (method class name (required-params [sym ...]
                                        [:opt sym-val [sym-val ...]]
                                        [:key sym-val [sym-val ...]]
                                        [:rest sym])
        body ...)
    required-params ::= sym [sym ...]
    sym-val ::= {sym | (sym val)}
    name -- メソッド名
    class -- メソッドを決定するクラス
    sym -- シンボル
