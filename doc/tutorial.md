...
ParenはS式によって記述されるプログラミング言語Lispの方言である。オブジェクトシステムなど着想の多くはCommon　Lispから受けている。Common Lispと最も異なる特徴を一つ上げるなら、Parenは手続き型言語としての側面が強いことである。一般に、代入が少ないプログラムはそうでない場合に比べてより堅牢であると考えられるが、一方で冗長になりがちである。Parenは代入のリスクを自己管理でき、必要に応じて使用する玄人のための言語である。勿論、Lispユーザを魅了してやまない玄人のための機能、マクロも使える。
...
2. replが印字した式の評価結果
を意味する。
write_line関数は評価されると引数を標準出力に印字し、評価結果として引数を返す。
...
関数の本体では、シンボルchにread_byteで標準入力から取得した文字を束縛している。その後、chに束縛されている文字をwrite_byteで標準出力に書き出している。
このプログラムは、
...
# コメント
コメントはParenに無視されるため、プログラムの有用な補足事項等、自由に記述することができる。
## 一行コメント
...
文字列リテラルはダブルクォートで囲まれた文字の列である。
    ...
    "hello paren"
オブジェクトから文字列を作成するにはto_strを使用する。to_str関数はParenのオブジェクトの印字表現を返す。
    ) (to_str 123)
    "123"
## 文字列の結合
文字列の結合は+を使用する。
...
# キーワード
キーワードは同名であるならばプログラムでただ一つだけ存在するという性質を持つオブジェクトである。キーワードを作成するには次のいずれかの方法を用いる。
- キーワードリテラルを使用する
- to_key関数を使用する
ただし、その性質上すでにシステムにその名前のキーワードが存在する場合、新しくオブジェクトは作られずに既存のオブジェクトへの参照が返る。
## キーワードリテラル
...
# シンボル
シンボルは処理の過程で任意のParenオブジェクトを束縛、参照するためのオブジェクトである。既に述べた数値や文字列の他にも関数やリスト、シンボルなどもParenのオブジェクトであるためシンボルに束縛することが可能である。
シンボルの束縛やスコープやエクステントなど、いくつかの項目は環境の章で改めて述べる。
## シンボル名
Parenのシンボル名は数字以外の英文字と一部の記号から始まり、その後英数字と一部の記号が続く。
次のいずれもParenのシンボル名として有効である。
    var
    x1
Parenでは様々な記号をシンボル名に使用することができる。次に挙げるシンボル名は(推奨しないが)すべて有効である。
    !$*+-0-9abc?
    m<__.__>m
シンボル名称で使用できる文字の厳密な定義は言語仕様書の字句解析の章を参照されたい。
一般に、複数の単語で構成されているシンボル名は単語の区切れをアンダースコアで区切る。
    user_name
    neighbor_node
## シンボルの束縛
あるオブジェクトをシンボルから参照できるようにする対応付けのことを束縛という。シンボルに値を束縛するにはParenの組み込みの代入オペレータ―`<-`を使用する。
    (<- variable_value ...) => result
    variable_value ::= var value
    var -- シンボル名
    value -- シンボルに束縛する値
    result -- 最後に束縛した値
代入オペレータ―はシンボルvarに束縛する値valを組にして受け取る。
Parenでは束縛する前にシンボルを宣言する必要はない。
代入オペレータ―は最後に束縛した値を返す。
    ) (<- a 0 b 1)
    1
代入オペレータ―で複数の束縛を行う場合は、
すべて左から順に評価されることが保証されている。
     ...
     6
     6
Parenが組み込みで使用しているシンボルに値を束縛することもできるが、
ほとんどの場合、ユーザの予期せぬ動作となってしまうだろう。
## 参照
シンボルは評価されると束縛されている値を返す。
    ) (<- a 3 b 4)
    4
    ) a
    3
    ...
    4
ただし、シンボルに値が束縛されていない場合はエラーとなる。
## 定数
宣言時に初期値を束縛し、それ以降は参照しかできないシンボルのことを定数という。
Parenには組み込みで次の定数が存在する。
    ...
    false 自身   偽の代表値
    nil   自身   空の代表値
定数に値を束縛しようとするとエラーになる。
Parenでは組み込み以外の定数を定義することができない。
そのため、定数として扱うシンボルは、
慣習として他のシンボルと見分けがつくように大文字のシンボル名を使用する。
    ) (<- PI 3.141592
          MAX_BUF_SIZE 256)
    256
定数として扱うシンボルの値は容易に更新できてしまうため、
プログラマが注意して扱う必要がある。
    ) (<- PI 3)
    3
...
## コンスの作成
リストを理解するためにはコンスを理解する必要がある。コンスとは次に定義されるParenのオブジェクトである。
    次の条件を満たすParenのオブジェクトへのポインタcar、cdrの組をコンスという。
    carとは、任意のParenのオブジェクトを指すポインタのことをいう。
    cdrとは、コンス又は定数nilを指すポインタのことをいう。

コンスは組み込み関数consにより作ることができる。consの第一引数、第二引数が、それぞれ作成されるコンスのcarとcdrに対応する。
    ) (cons 1 nil)
    (1)
    ) (cons 1 (cons 2 nil))
    (1 2)
    ...
    ((1) 2)
コンスの定義にあてはまらない引数をconsに与えるとエラーとなる。
Parenはコンスを次の手順で印字する。
1. 左括弧を印字
2. carが指す値を印字する
   cdrが指す値がnilなら3を行う
    ...
    +-----+-----+
       ...
       +--->| car | cdr |          3
            +-----+-----+
               ...
               |     +---> nil
               1
このように、あるコンスのcdrで辿れるコンス全体が括弧で囲われて印字される。コンスのcdrで辿れるコンス全体のことをリストという。これは、辿れるコンス全体が単方向リストと見做せるからである。下に、その対応関係を示す。
    値   対応
    ---------------------------------------------
    cons 単方向リストのセル
...
あるコンスのcdrを辿っていった終端がnilを指している場合、そのコンス全体は純リストであるという。コンスの定義により、Parenの任意のリストはすべて純リストである。純リスト以外のリストが作れないという制約はほかのLisp系の言語と比べると奇妙かもしれない。
...
この二つの関数は空を表す擬似シンボルnilを与えた場合を除き同じ結果を返す。
空を表す擬似シンボルnilに対してlist?がtrueを返すように実装されている。
以後、nilが要素がないリストの意味で用いる場合は空のリストと呼ぶ。
...
# 環境
Parenがシンボルとそれに対応づけられた値との関係を、
環境と呼ばれるモデルを採用して管理している。
Parenのシンボルについて理解するために、環境の理解は必須となる。
## 環境モデル
式が評価される場合、必ずどこかの環境の下で評価される。
環境とは、シンボルと値の一対一の対応の集合と、
...
このように、あるシンボルの評価結果は評価される環境によって異なる。
## 束縛規則
Parenの起動直後の環境はトップレベルであるから、
今までの束縛の例はすべてトップレベルでの束縛であった。
トップレベル以外の環境も考慮すると、束束縛は次の規則で処理される。
    束縛するシンボルが束縛されている、現在の環境を含む最も近い親の環境を探し、
    その環境で束縛されている値を更新する。
    ただし、シンボルが束縛されていない場合はトップレベルに新しく束縛する。
束縛の簡単な例を下に示す。
先ほどの例と同じようにa、b、cをトップレベルで束縛する。
    ...
    6
ここで、letでトップレベルの下に新しく環境を作成しその中で束縛を行う。
    ) (let ((a 1) (b 2))
          (* (<- a -1) (<- b -2) (<- c -3)))
    -6
再度トップレベルで積をとると、
評価結果が最初の値と変わっていることが確認できる。
    ...
    -6
これは、新しく作成した環境でcに値を束縛したとき、
トップレベルで束縛されているcの値が更新されたからである。
## スコープ
あるシンボルを参照可能な範囲のことをシンボルのスコープ乃至、可視範囲という。
Parenのシンボルのスコープは、シンボルの評価規則に従い次のようになる。
    あるシンボルのスコープはそのシンボルが束縛されている環境から辿れるすべての子の環境である。
    ただし、辿りうる子の環境の中に同名のシンボルが束縛されている場合は、
    シンボルのスコープはその環境よりも親の環境までとなる。
親の環境で束縛されているシンボルを、子の環境で束縛することを、
シンボルの再束縛、乃至、シンボルのシャドーイングという。
## エクステント
...
Parenは無限エクステントであるから、
あるシンボルの参照先が既に破棄されていたというようなことはおこらない。
ただし、一旦メモリに確保されたオブジェクトが、
永遠に破棄されないというわけではない。
プログラムから参照する術がなくなったオブジェクトは、
ガーベージコレクションにより定期的に破棄される。
## グローバルシンボル
Parenではトップレベルで束縛されたシンボルのことをグローバルシンボルという。
グローバルシンボルは再束縛されていないすべての環境から参照できる。
慣習として、ほかのシンボルと区別がつきやすいようにグローバルシンボル名は`$`から始める。
    $global_var
組み込みの代表的なグローバルシンボルを下に示す。
    シンボル名    値
    ---------------------------------------------
...
Common Lispでシンボルの束縛を行う場合、次のロジックで処理が行われる。
    ...
    3. 親の環境にシンボルが束縛されていなかった場合はトップレベルにシンボルを束縛する。
