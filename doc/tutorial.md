...
`<-`は現在の環境にシンボルとシンボルが保持するオブジェクトへの参照の組を束縛する。既に現在の環境にシンボルが束縛されている場合はシンボルテーブルの対応する値を更新する。

`<--`は現在の環境にシンボルが束縛されていない場合に、親の環境に対して再帰的に探索を続けてシンボルテーブルを更新するという点を除いて`<-`と同じである。ただし、大域環境にもシンボルが束縛されていない場合は大域環境に束縛する。
...
## 環境の作成
Parenは関数オブジェクトが環境を持つ。つまり、新しく環境を作るということは関数オブジェクトを作成することに他ならない。このことについては関数の章で述べる。
...
## コンス
リストとは、コンスのcdrで辿れるコンス全体、または、nilのことをいう。空の代表値nilをリストの文脈で用いる場合、空のリストということがある。。
...
コンスは組み込み関数consで作ることができる。consの第一引数、第二引数が、それぞれ作成されるコンスのcarとcdrに対応する。
...
コンスの印字表現はリストを強く意識している。このことについては後述する。
