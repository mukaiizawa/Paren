...
(method ParenLexer -identifier-lead? ()
  (let (c (&next self))
...
(method ParenLexer -identifier-trail? ()
  (or (-identifier-lead? self) (ascii-digit? (&next self))))
...
(method ParenLexer -lex-number (sign)
    ...
    (let (next (&next self))
      (if (.eof? self) '(:EOF)
          ...
          (begin (if (find '(0x2B 0x2D) next :test =)
                     (<- sign (.skip self)))
                 nil) :unreachable
          (ascii-digit? (&next self)) (list :number (-lex-number self sign))
          (or sign (-identifier-lead? self)) (begin
                                               (if sign (.put self sign))
                                               (list :symbol (-lex-symbol self)))
          (error " illegal char")))))
