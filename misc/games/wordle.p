...
(function write-gray (ch)
  (console.write ch :color :white :bg-color :black))

(function write-yellow (ch)
  (console.write ch :color :white :bg-color :yellow))

(function write-green (ch)
  (console.write ch :color :white :bg-color :green))

(function write-hyphen ()
...
(function refresh ()
  ...
  (dotimes (i $max-times)
    (let (word ([] (reverse $histories) i))
      ...
      (dotimes (j 5)
        (let (ch ([] word j))
          (if (= ch ([] $word j)) (write-green ch)
              (in? ch $word) (write-yellow ch)
              (empty? ch) (write-gray " ")
              (write-gray ch)))
        (console.write "|"))
      (console.write "\n"))
