...
(function! main (args)
  ...
  (let (re (Regex.compile "a{0,}"))
    ...
    (assert (.match? re "aa")))
  ; api
  (memeq? (regex.match "o+" "fooo") "oo")
  (nil? (regex.match "o{3}" "foo"))
  (let ((:opt foo bar buzz) (regex.match-all "[fb].*?," "foo,bar,buzz"))
    (assert (memeq? foo "foo,"))
    (assert (memeq? bar "bar,"))
    (assert (nil? buzz)))
  (let (re (Regex.compile "[,:]"))
    (let ((:opt foo bar buzz) (regex.split re "foo,bar,buzz"))
      (assert (memeq? foo "foo"))
      (assert (memeq? bar "bar"))
      (assert (memeq? buzz "buzz")))
    (let ((foo bar-buzz) (regex.split re "foo:bar:buzz" 1))
      (assert (memeq? foo "foo"))
      (assert (memeq? bar-buzz "bar:buzz"))))
  (let (re (Regex.compile "[,:]"))
    (assert (memeq? (regex.replace re "foo,bar,buzz" "|||") "foo|||bar|||buzz"))
    (assert (memeq? (regex.replace re "foo:bar:buzz" "|||" 1) "foo|||bar:buzz"))))
