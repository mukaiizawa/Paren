...
(function Regex.parse-charset (ar)
  (.skip ar)
  (.reset ar)
  (let (key (if (string-eq? (&next ar) "^") (begin (.skip ar) :exclude-char-class) :char-class))
    (while (! (string-eq? (&next ar) "]"))
      (<- c (.skip-escape ar))
      (if (string-eq? (&next ar) "-")
          (begin (.skip ar)
...
(function Regex.parse-quantifier (ar expr)
  (let (c (&next ar) n 1 m 1 greedy? true)
    (if (string-eq? c "*") (begin (.skip ar) (<- n 0 m nil))
        (string-eq? c "?") (begin (.skip ar) (<- n 0 m 1))
        (string-eq? c "+") (begin (.skip ar) (<- n 1 m nil))
        (string-eq? c "{") (begin (.skip ar)
                                  (<- n (.skip-unsigned-integer ar))
                                  (if (string-eq? (&next ar) "}") (<- m n)
                                      (begin (.ensured-skip ar ",")
                                             (if (string-eq? (&next ar) "}") (<- m nil)
                                                 (<- m (.skip-unsigned-integer ar)))))
                                  (.ensured-skip ar "}")))
    (when (string-eq? (&next ar) "?")
      (.skip ar)
      (<- greedy? nil))
...
(function Regex.parse (ar)
  (let (elements nil expr nil c nil)
    (while (&& (<- c (&next ar)) (! (string-index "|)" c)))
      (<- expr (if (string-eq? c ".") (Regex.parse-any ar)
                   (string-eq? c "(") (Regex.parse-group ar)
                   (string-eq? c "[") (Regex.parse-charset ar)
                   (Regex.parse-char ar)))
...
(method Regex .test (elt i)
  (if (< i (.text-length self))
      (switch (&key elt)
        :char (if (string-eq? (.text-at self i) (&val elt)) (++ i))
...
(method Regex .match? (s :opt start)
  ...
  (if (&anchored-start? self)
      (return (&& (= start 0) (.try self (&elements self) 0) true)))
