...
(function dom.attributes (node)
  (select symbol? (cadr node)))

(function dom.attribute (node attr)
  (cadr (member attr (cadr node))))
...
(function dom.query-selector-all (dom selector)
  (let (nodes nil selectors (dom.compile-selector selector)
              ...
              basic-match? (f (node expr)
                             (every? (f (x)
                                       (let ((type args) x)
                                         (if (== type :name) (|| (= args "*") (== (symbol args) (dom.name node)))
                                             (== type :id) (= args (string (dom.id node)))
                                             (== type :class) (in? args (dom.classes node))
                                             (== type :attr) (in? (symbol args) (dom.attributes node))
                                             (== type :attr=) (= (dom.attribute node (symbol (car args))) (cadr args))
                                             (== type :attr^=) (prefix? (dom.attribute node (symbol (car args))) (cadr args))
                                             (== type :attr$=) (suffix? (dom.attribute node (symbol (car args))) (cadr args))
