...
(method Path .copy (dst)
  (if (.file? self) (.contents dst (.contents self))
      (begin
        (.mkdir dst)
        (dolist (file (.children self))
          (.copy file (.resolve dst (.name file)))))))
...
(method Stream .read-char ()
  ...
  (chr
    (let (byte (.read-byte self))
      (if (< byte 0) (return nil)
          (< byte 0x80) byte
          (< byte 0xe0) (| (<< (& byte 0x3f) 6)
                           (& (.read-byte self) 0x3f))
          (< byte 0xf0) (| (<< (& byte 0xf) 12)
                           (<< (& (.read-byte self) 0x3f) 6)
                           (& (.read-byte self) 0x3f))
          (< byte 0xf8) (| (<< (& byte 0x3) 18)
