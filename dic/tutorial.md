...
より厳密には、複数行注釈内で`|#`を使用できない。
    #|
     ...
     => Error
## Parenドキュメント
複数行注釈のうち、`#||`で始まるものをParenドキュメントという。
Parenにはソースコード中のParenドキュメントをhtml形式に変換する機能がある。
モジュールを作成する際はParenドキュメントを記載することが望ましい。
Parenドキュメントの詳細は別紙を参照のこと。
...
コンスは組み込み関数consにより作ることができる。
    ...
    => (2 1)
consの第一引数、第二引数が、
それぞれ作成されるコンスのcarとcdrにあたる。
コンスまたは恒等変数:nilのことをリストという。
任意のコンスのcarはParenのオブジェクトなら何でも指せるが、
任意のコンスのcdrはリストを指していなければならない。
    (cons 1 2)
    => Error
...
carとcdrは引数が:nilの場合は:nilを返す。
    ...
    (car (cdr (cdr lis)))
    => :nil
関数carとcdrの動作から:nilは要素のないリストと見做せる。
そのようなリストを空のリストと呼ぶ。
実際:nilは空のリストを表すように定義された恒等変数である。
...
### オプショナルパラメーター
オプショナルパラメーターは、
実引数を省略した場合には指定した初期値を用いて関数を実行するための仕組みである。
仮引数に`?`を指定した以降のパラメータは、
オプショナルパラメーター名、初期値の対となり、
呼び出し時に省略された場合はその初期値が使用される。
先ほど使用したlinearをオプショナル引数を用いて定義した場合について示す。
    (def linear (a ? x 1 b 0)
      (+ (* a x) b))
    (linear 3)
    => 3
    (linear 3 2)
...
初期値を指定しない場合はエラーとなってしまう。
    (def linear (a ? x 1 b)
      (+ (* a x) b))
    => Error
また、i番目のオプショナルパラメーターを指定したい場合は、
i - 1番目までのオプショナルパラメータをすべて指定しなければならない。
### レストパラメーター
