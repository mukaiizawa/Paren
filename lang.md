...
# 基本概念
ParenはCommon Lisp、Arcをはじめ、
様々なプログラミング言語にインスパイアされた超軽量スクリプト言語である。
...
### コメント(comment)
    comment = (';' [^\n]* | '#|' .*  '|#')
コメントはセミコロンから行末までである。
もしくは、改行を含める`#|`で始まり`|#`で終わる任意の文字列で表現される。
## S式(s_expr)
    s_expr = list | atom
S式はリストまたはアトムである。
### リスト(list)
...
処理の簡潔化のために`001`等も数字と見做し、`1`と評価する。
#### 文字(char)
    char = '\'' (. | esc) '\''
...
`fn`は無名関数を定義する。
第一引数がシンボルの場合は、
関数呼び出し時に実引数すべてがリストとしてそのシンボルに束縛される。
    ((fn x x) 1 2 3)
    => (1 2 3)
上の例ではすべての実引数が`x`にリストとして束縛される。
第一引数がリストの場合はリスト内のシンボルが、
実引数に束縛される。
    ((fn (x y z) (* (+ x y) z)) 1 2 3)
    => 9
また、最後の仮引数の手前に`.`を表記することにより、
残りの実引数をリストで受け取る関数を定義できる。
    ((fn (fn . rest) (apply fn rest)) * 1 2 3)
    => 6
第一引数にキーワードを指定することにより、
総称関数を定義することができる。
    (def put)
    (<- put (fn :Stream (x y) ...)
        put (fn :Set (s v) ...))
ここで、`put`の第一実引数の型により、どの関数が呼び出されるかが動的にきまる。
関数は第一引数と強く結びつくため、
...
したがって、次のメソッド`put`が定義されている場合には、
    (defun :Stream (put stream str)
           ...)
次のような、シグネチャが重複するメソッドは定義できない。
    (defun :Stream (put stream char n)
       ...)
    => error.
...
次のようにシグネチャが異なる場合は総称関数を定義することができる。
    ...
    (defun :Map (put m obj)
       ...)
    ;; メソッド名称が異なる
    (defun :Stream (putLn s str)
       ...)
関数呼び出しは呼び出す際の第一引数の型により動的に割り当てられる。
    ;; xの型により、メソッドputが定まり実行!!
    (put x y)
