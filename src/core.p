...
第一引数が変数束縛のListを表す。
第二引数移行が、第一引数で束縛した変数の環境下にて暗黙のprognにより評価される。
は後述する暗黙のprognにより実行される。
## progn(逐次実行)
...
その利用頻度から構文糖である、バッククォートリードマクロが定義されている。
# fn(関数定義)
((fn (Object o)
     (asString o))
 1)
...
# S式の評価
(f arg1 arg2... argN)
フォームはarg1, arg2, ... , argNの順で評価され、
最後にfの仮引数に評価結果のarg1, arg2, ... , argNが渡されてfが評価される。
